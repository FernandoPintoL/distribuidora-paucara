/**
 * Componente: Botón de Impresión para Listado de Proformas
 *
 * Modal Dialog para seleccionar formato de impresión.
...
interface FiltrosProformas {
    ...
    totalMin?: string;
    totalMax?: string;
    filtroVencidas?: string;
    [key: string]: any;
}

interface ImprimirProformasButtonProps {
    proformas: Proforma[];
    filtros?: FiltrosProformas;
    className?: string;
    iconOnly?: boolean;
}

const FORMATOS_PROFORMAS = [
...
export function ImprimirProformasButton({
    proformas,
    filtros = {},
    className = '',
    iconOnly = false,
}: ImprimirProformasButtonProps) {
    ...
    const prepararImpresion = async (formato: string, accionURL: 'download' | 'stream' = 'stream') => {
        console.log('Preparando impresión de proformas:', { formato, filtros, accion: accionURL });
        setLoading(true);
        ...
        try {
            ...
            const response = await fetch('/api/proformas/preparar-impresion', {
                method: 'POST',
                headers: {
            ...
            console.log('✅ Backend preparó impresión:', {
                cantidad: responseData.cantidad_proformas,
                ids: responseData.proforma_ids,
                mensaje: responseData.message
            });

            // ✅ MEJORADO: Usar ruta específica para impresión filtrada (no será capturada por rutas dinámicas)
            const idsParam = responseData.proforma_ids.join(',');
            const url = `/proformas/imprimir-filtrado?formato=${formato}&accion=${accionURL}&ids=${idsParam}`;
            ...
            NotificationService.success('Reporte de proformas enviado');
            handleClose();
        } catch (error) {
    ...
    // No mostrar si no hay proformas
        ...
        <>
            {iconOnly ? (
                ...
                >
                    ...
                    {loading ? 'Generando...' : 'Imprimir Filtrado'}
                </Button>
            )}
                                ...
                                <Dialog.Panel className="w-full max-w-md transform overflow-hidden rounded-2xl bg-white dark:bg-zinc-900 p-6 text-left align-middle shadow-xl transition-all">
                                    ...
                                    <div className="flex items-center justify-between mb-6">
                                        <div className="flex items-center gap-3">
                                            ...
                                            <Dialog.Title className="text-xl font-semibold text-gray-900 dark:text-white">
                                                {accion ? 'Seleccionar formato' : 'Reporte de Proformas'}
                                            </Dialog.Title>
                                        </div>
                                        <button
                                            onClick={handleClose}
                                    ...
                                    <div className="mb-6 p-3 bg-blue-50 dark:bg-blue-900/20 rounded-lg border border-blue-200 dark:border-blue-800">
                                        <p className="text-sm text-blue-900 dark:text-blue-300">
                                            Registros en pantalla: <span className="font-semibold">{proformas.length}</span>
                                        </p>
                                        ...
                                        <div className="space-y-6">
                                            <div>
                                                ...
                                                <div className="space-y-2">
                                                    {FORMATOS_PROFORMAS.map((formato) => (
                                                        <button
                                                            key={formato.formato}
