...
interface DevolucionClienteDetalle {
    id: number;
    devolucion_cliente_id: number;
...
interface DevolucionClienteHeader {
    id: number;
...
interface PrestamroConDevoluciones {
    id: number;
    cliente: {
        id: number;
        nombre?: string;
    ...
    detalles: any[];
    devoluciones: DevolucionClienteHeader[];
}

export default function DevolucionesPage({ prestamoId }: { prestamoId: number }) {
    const [prestamo, setPrestamo] = useState<PrestamroConDevoluciones | null>(null);
    const [loading, setLoading] = useState(true);
    const [toasts, setToasts] = useState<Array<{ id: string; message: string; type: 'success' | 'error' | 'warning' | 'info' }>>([]);
    const [mostrarModalImpresion, setMostrarModalImpresion] = useState(false);
    const [devolucionSeleccionada, setDevolucionSeleccionada] = useState<DevolucionClienteHeader | null>(null);
    ...
    const imprimirDevolucion = (devolucionId: number) => {
        ...
        const url = `/api/prestamos-cliente/${prestamoId}/devoluciones/${devolucionId}/imprimir`;
        window.open(url, '_blank');
    };

    const abrirDetalles = (devolucion: DevolucionClienteHeader) => {
        console.log('=== DEVOLUCION SELECCIONADA ===');
        ...
        devolucion.detalles.forEach((detalle, idx) => {
            ...
            console.log('  DetallePrestamoCliente:', detalle.detalle_prestamo_cliente);
            console.log('  Prestable:', detalle.detalle_prestamo_cliente?.prestable);
            console.log('  Nombre Prestable:', detalle.detalle_prestamo_cliente?.prestable?.nombre);
        });
        console.log('================================\n');
    ...
    const calcularTotalCantidadesDevuelta = (devolucion: DevolucionClienteHeader) => {
        return devolucion.detalles.reduce((sum, d) => sum + d.cantidad_devuelta, 0);
    };

    const calcularTotalCantidadesDañada = (devolucion: DevolucionClienteHeader) => {
    ...
    if (loading) {
        return (
            <AppLayout>
                <Head title="Devoluciones del Préstamo" />
            ...
            <div className="p-8 bg-white dark:bg-gray-950 min-h-screen">
                ...
                <div className="flex items-center justify-between mb-8">
                    <div>
                        ...
                        <p className="text-sm text-gray-600 dark:text-gray-400 mt-2">
                            Préstamo #{prestamo.id} - {prestamo.cliente.nombre || prestamo.cliente.razon_social}
                        </p>
                    </div>
                ...
                {obtenerTotalDevoluciones() === 0 ? (
                    ...
                    <div className="space-y-4">
                        {prestamo.devoluciones.map((devolucion: DevolucionClienteHeader, idx: number) => (
                                ...
                                <div>
                                    ...
                                    <div className="border rounded-lg overflow-x-auto">
                                        <Table>
                                            ...
                                            <TableBody>
                                                {devolucionSeleccionada.detalles.map((detalle: DevolucionClienteDetalle) => (
                                                    <TableRow key={detalle.id}>
                                                        <TableCell className="font-medium">
                                                            {detalle.detalle_prestamo_cliente?.prestable?.nombre || 'Prestable desconocido'}
                                                            <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
                                                                Tipo: {detalle.detalle_prestamo_cliente?.prestable?.tipo || 'N/D'}
                                                            </p>
                                                        </TableCell>
                ...
                <OutputSelectionModal
                    ...
                    tipoDocumento="devoluciones-cliente"
                    documentoInfo={{
                        numero: `#${prestamo?.id}`,
