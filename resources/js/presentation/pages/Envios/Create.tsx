...
import { AlertCircle, CheckCircle2, Truck, User, MapPin, Calendar, Clock } from 'lucide-react';
import enviosSearchService from '@/infrastructure/services/envios-search.service';
...
export default function Create({ ventas, vehiculos, choferes, ventaPreseleccionada }: Props) {
    ...
    useEffect(() => {
        ...
        if (historial) {
            try {
                ...
                console.error('Error cargando historial de choferes:', e);
            }
        }
    }, [choferes]);
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        // Validar fecha
        if (!enviosSearchService.validarFecha(data.fecha_programada)) {
            setFechaError(enviosSearchService.getMensajeFechaInvalida(data.fecha_programada));
            return;
        }
        ...
        if (data.chofer_id) {
            const historial = localStorage.getItem('envios_ultimos_choferes');
            let chofersIds: number[] = historial ? JSON.parse(historial) : [];
            const choferIdNum = parseInt(data.chofer_id);
            chofersIds = [choferIdNum, ...chofersIds.filter(id => id !== choferIdNum)].slice(0, 5);
            localStorage.setItem('envios_ultimos_choferes', JSON.stringify(chofersIds));
        }
    ...
    // Convertir datos a opciones para SearchSelect (locales)
    ...
    const chofersConHistorial = useMemo(() => {
        const sinHistorial = chofersOptions.filter(
            c => !historialChoferes.some(h => h.value === c.value)
        );
        return [...historialChoferes, ...sinHistorial];
    }, [chofersOptions, historialChoferes]);

    // Validar si todos los campos requeridos están completos
    const isFormValid = data.venta_id && data.vehiculo_id && data.chofer_id && data.fecha_programada && data.direccion_entrega;

    // Generar fecha mínima (1 hora desde ahora)
                            ...
                            {selectedVenta ? (
                                <Card className="sticky top-6 border-2 border-blue-200 dark:border-blue-900">
                                    <CardHeader className="bg-blue-50 dark:bg-blue-950">
                                        <CardTitle className="text-sm">Resumen del Envío</CardTitle>
                                    </CardHeader>
                                    <CardContent className="space-y-4 pt-4">
                                        ...
                                        {selectedVehiculo && (
                                            <div className="pb-4 border-b">
                                                ...
                                                <Badge variant="outline" className="mt-2 text-xs">
                                                    {selectedVehiculo.capacidad_carga}kg
                                                </Badge>
                                            </div>
                                        )}
                                        ...
                                        {selectedChofer && (
                                            <div className="pb-4 border-b">
                                                ...
                                                <p className="text-sm font-medium mt-1">{selectedChofer.name}</p>
                                ...
                                <Card className="border-dashed border-2">
                                    <CardContent className="flex flex-col items-center justify-center py-8 text-center">
                                        ...
                                        <p className="text-sm text-muted-foreground">
                                            Selecciona una venta para ver el resumen del envío
                                        </p>
                                    </CardContent>
                                </Card>
                            )}
