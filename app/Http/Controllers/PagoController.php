...
class PagoController extends Controller
{
    public function index(Request $request)
    {
        $query = Pago::with(['cuentaPorPagar.compra.proveedor', 'tipoPago', 'usuario'])
            ...
            ->when($request->search, function ($q) use ($request) {
                $q->where(function ($subQ) use ($request) {
                    $subQ->where('numero_recibo', 'LIKE', "%{$request->search}%")
                        ->orWhere('numero_transferencia', 'LIKE', "%{$request->search}%")
        ...
        return Inertia::render('compras/pagos/index', [
            ...
            'filtros' => $request->only(['tipo_pago_id', 'fecha_desde', 'fecha_hasta', 'search']),
